...
**waggle はもう BrowserHive と gRPC で話しません。** 投げて待って報告するのは
Windmill の flow で、waggle は次の段を組み立てて記録します。waggle の設定に
BrowserHive の在り処は 1 つも入っていません。

1 段が 1 往復です。flow は渡された URL を取り込み、`POST /api/crawls/:id/pages` に
ページごとの `taskId` と状態を載せて返します。waggle は `crawl_pages` を書き、
帰属を記録し、見つかったリンクを範囲で絞り、上限を当て、次の段を答えます ——
あるいは `stopReason` を返します。

段の報告に**成果物の在り処は載っていません**。そこで waggle は、BrowserHive が
...
- **`failed` と報告されたページでも、アーカイブができていることがあります。**
  ...
  受け取って失敗と報告しますが、manifest は bucket に在るので waggle が拾い直し、
...
- 台帳へ入る道は 2 本あります。**クロール**が段で取り込んだぶんを載せる経路
  ...
  （waggle が止まっていた間の取りこぼしを拾う）。どちらも `admitArchive` を通り、
...
| `src/config/capture-formats.ts` | `WAGGLE_CAPTURE_FORMATS` / `_SIGNING` を、毎回載せる `captureFormats` にする。                   |
...
あちらだからです。waggle の側の関心事は、`crawls` の行がどうなるかです。
