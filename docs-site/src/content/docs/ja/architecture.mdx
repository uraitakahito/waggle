...
flowchart LR
  ...
  WM -->|Capture| BH1
  WM -->|Capture| BH2
  subgraph BH["BrowserHive — browser 1 台に 1 プロセス"]
    BH1[browserhive-1]
    BH2[browserhive-2]
  end
...
**capture-ledger はもう BrowserHive と gRPC で話しません。** `Capture` を呼んで答えを
待ち、報告するのは Windmill の flow で、capture-ledger は次の段を組み立てて記録します。
capture-ledger の設定に BrowserHive の在り処は 1 つも入っていません。BrowserHive は
1 プロセスが browser 1 台を持ち、取り込みは 1 度に 1 つ —— 走行中なら
`RESOURCE_EXHAUSTED` で断る —— ので、空いている口を選ぶのも flow の仕事です。
...
この file は BrowserHive の再起動にも耐えるので、ここで使えます。
