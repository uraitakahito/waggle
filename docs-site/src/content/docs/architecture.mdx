---
title: Architecture
description: How waggle, Postgres, BrowserHive and the artifact store fit together.
---

## Stage 0 — where the project is today

```mermaid
flowchart LR
  PG[("Postgres<br/>urls table")] -->|SELECT url, labels| W
  subgraph W["waggle (Node 24 / TypeScript)"]
    direction TB
    US[url-source<br/>pg query] --> CO[cli-options<br/>flags → CaptureSettings]
    CO --> RUN[run.ts → submit.ts<br/>parallel]
    RUN --> SDK[generated SDK<br/>hey-api / fetch]
  end
  SDK -->|POST /v1/captures| BH
  subgraph BH["BrowserHive"]
    COORD[coordinator + worker pool]
  end
  COORD -->|CDP| C1[chromium-server-1]
  COORD -->|CDP| C2[chromium-server-2]
  COORD -->|PutObject| S3[(SeaweedFS<br/>bucket: browserhive)]
  W -.->|log only| OUT["Request accepted × N<br/>Request summary"]
```

waggle's half of that picture is small on purpose. Everything to the right of
`POST /v1/captures` is BrowserHive's, and is documented
[there](https://uraitakahito.github.io/browserhive/).

## Fire-and-forget

BrowserHive answers **202** with a `taskId` as soon as the request is queued, and
runs the capture asynchronously. waggle records which submissions were accepted
and exits.

waggle itself exposes no endpoint for the outcome, but BrowserHive does:
`GET /v1/captures/{taskId}` answers `202` while a task is still in flight and
`200` once it finished, with the status and every artifact location. The same
body is also written to the bucket as `{taskId}_..._{labels}.result.json`,
which survives a BrowserHive restart. See
[Capture results](https://uraitakahito.github.io/browserhive/capture-results/).

waggle does not poll it today — it records which submissions were accepted and
exits.

Consequences worth knowing:

- A `Request summary` of `accepted: 3` means three tasks were **queued**, not
  that three captures succeeded.
- Failures after acceptance are invisible to waggle. Lifecycle tracking is
  Stage 1 of the roadmap.
- `correlationId` (8 hex characters, generated per submission) is the thread
  between a waggle log line and the artifact filename.

## Module layout

| Module                      | Responsibility                                                                         |
| --------------------------- | -------------------------------------------------------------------------------------- |
| `src/cli.ts`                | Entry point. Parses, runs, exits non-zero on a fatal error.                            |
| `src/config/cli-options.ts` | Flag definitions, `ClientOptions`, and the `CaptureSettings` the request body spreads. |
| `src/data/url-source.ts`    | The one SQL query. Returns `{ url, labels }[]`.                                        |
| `src/client/run.ts`         | Orchestration: load, configure, submit all, summarise.                                 |
| `src/client/submit.ts`      | One request. Never throws — failures come back as `accepted: false`.                   |
| `src/http/generated/`       | Generated from the vendored OpenAPI spec. Never edited by hand.                        |
| `src/db/`                   | Pool, Kysely wiring, migrations, seeds.                                                |

## Error handling

Both HTTP failures (RFC 7807 problem responses) and transport failures surface
as `accepted: false` with a message — `submitRequest` never throws at the caller.
The message prefers `detail` over `title`, so a rejected request reports the
field that was wrong rather than a bare status code.

## Network layout

The stack runs on Apple Container. The project name is the DNS domain, so every
service is `<service>.waggle` — resolvable between containers **and from the
host**, which is why waggle itself needs no container.

| Service     | DNS name             | Container port     | Host port         |
| ----------- | -------------------- | ------------------ | ----------------- |
| postgres    | `postgres.waggle`    | 5432               | 5432              |
| seaweedfs   | `seaweedfs.waggle`   | 8333 / 8888 / 9333 | — (internal only) |
| chromium-1  | `chromium-1.waggle`  | 9222 (CDP)         | 9222              |
| chromium-2  | `chromium-2.waggle`  | 9222 (CDP)         | 9223              |
| browserhive | `browserhive.waggle` | 8080               | 8080              |
| waggle      | — (runs on the host) | —                  | —                 |

There is one compose file. The one-shot migrate/seed/capture jobs are not
services: `scripts/prod-smoke.sh` invokes them with `container run --rm`,
because container-compose has no `run` subcommand.

Bucket creation is **not** a separate service. BrowserHive's SeaweedFS entrypoint
spawns its `init-bucket.sh` alongside `weed server`, and its retry loop does the
waiting. `browserhive` is given `WAIT_FOR_S3`; its entrypoint polls that URL
until the S3 listener answers before starting the server, because the boot
`HeadBucket` is fatal and `depends_on` orders starts only.

## Upstream pinning

waggle tracks a fixed BrowserHive version through one submodule pointer — see
[Upgrading BrowserHive](/waggle/upgrading-browserhive/), which reads the current
value out of the repository rather than restating it.
