...
**waggle does not speak gRPC to BrowserHive any more.** The Windmill flow
submits, waits, and reports; waggle plans the next level and records what
happened. There is no BrowserHive address in waggle's configuration at all.

One level is one round trip. The flow captures the URLs it was handed and posts
`POST /api/crawls/:id/pages` with a `taskId` and a status per page. waggle writes
...
The level report does **not** carry artifact locations, so waggle re-reads the
`{taskId}_{correlationId}[_{labels}].result.json` manifest BrowserHive writes
next to the artifacts. That file survives a BrowserHive restart and outlives the
server's bounded result cache, which is what makes it usable here.
...
- **A page reported `failed` may still have produced an archive.** If the result
  ...
  waggle admits it anyway and corrects the `crawl_pages` row.
...
- Two paths lead into the ledger: a **crawl** admitting what a level captured
  ...
  up whatever landed while waggle was down). Both go through `admitArchive`, and
...
| `src/config/capture-formats.ts` | `WAGGLE_CAPTURE_FORMATS` / `_SIGNING` → the `captureFormats` every dispatch carries.                       |
...
holding the gRPC channel. waggle's half is what happens to the `crawls` row.
